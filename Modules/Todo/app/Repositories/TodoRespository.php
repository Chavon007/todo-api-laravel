...
class TodoRepository implements TodoRespositoryInterface{
     public function all() {return Todo::all();} 
    public function find(int $id) {return Todo::findOrFail($id);}
    public function create(array $data) {return Todo::craete($data);}
    public function update(int $id, array $data) {$todo = Todo::findOrFail($id); $todo -> update($data); return $todo;}
